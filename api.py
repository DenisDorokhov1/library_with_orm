from sqlalchemy import Column, Integer, Date, String, Float,\
      Boolean, Date, create_engine, ForeignKey, func, extract
...
def insert_data():
   ...
   students = [Student(name="Nikita", surname="Ivanov", phone="2", email="3",
                       average_score=4.5,
                       scholarship=True),
               Student(name="Vlad", surname="Petrov", phone="2", email="3",
                       average_score=3.5,
                       scholarship=False),
                Student(name="Denis", surname="D", phone="3", email="5",
                average_score=4.1, scholarship=True),
                Student(name="Olga", surname="Davis", phone="6", email="7",
                        average_score=4.5, scholarship=False)
               ]
...
def give_me_book():
   ...
   books_to_nik = session.query(Book).join(Author).filter(Author.surname == 'Толстой').all()
   ...
   for book in books_to_nik:
       if book.count <= 0:
           print(f"Предупреждение: книгу «{book.name}» (id={book.id}) брать нельзя — в наличии 0 экземпляров.")
           continue
